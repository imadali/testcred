...
if ($email_type == 'archived') {
	...
	$xtpl=new XTemplate ('custom/modules/Emails/DetailView.html');
} else {
	$xtpl=new XTemplate ('custom/modules/Emails/DetailViewSent.html');
	if($focus->type == 'out') {
...
if (!empty($focus->parent_type) && $focus->parent_type !='test') {
	...
	$xtpl->assign('PARENT_TYPE_UNTRANSLATE', $focus->parent_type);
        if(isset($app_list_strings['record_type_display'][$focus->parent_type])){
                $xtpl->assign('PARENT_TYPE', $app_list_strings['record_type_display'][$focus->parent_type] . ':');
            } else {
                $xtpl->assign('PARENT_TYPE','');
            }
        }
...
if(!isset($app_strings['LBL_EDIT_BUTTON_LABEL'])) $app_strings['LBL_EDIT_BUTTON_LABEL']='';
if(!isset($app_strings['LBL_DELETE_BUTTON_TITLE'])) $app_strings['LBL_DELETE_BUTTON_TITLE']='';
if(!isset($app_strings['LBL_DELETE_BUTTON_LABEL'])) $app_strings['LBL_DELETE_BUTTON_LABEL']='';
if(!isset($app_strings['LBL_DELETE_BUTTON'])) $app_strings['LBL_EDIT_BUTTON_LABEL']='';
...
for($i=0; $i<count($notes_list); $i++) {
	$the_note = $notes_list[$i];
        $filePath = rtrim($GLOBALS['sugar_config']['upload_dir'],'/').'/'.$the_note->id;
        if(!empty($the_note->filename) && $the_note->file_mime_type == 'application/pdf' && file_exists($filePath)){
            $doc_id = $the_note->id;
            $pdf_path = $GLOBALS['sugar_config']['upload_dir'].$doc_id;
            $attachments .= '<a href="javascript:void(0);" onclick="window.open(\'index.php?entryPoint=customMergeDownload&id='.$the_note->id.'&type=Notes\', \'_blank\');">'.$the_note->name."</a><br />";
        }
	else if(!empty($the_note->filename))
