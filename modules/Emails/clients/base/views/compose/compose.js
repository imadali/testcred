({
    ...
    _render: function () {
        ...
        if (this.model.isNotEmpty) {
            ...
            if (!_.isEmpty(prepopulateValues)) {
                ...
                this.send_documents = prepopulateValues.send_documents;
                ...
                if (prepopulateValues.preFillAttachement) {
                    ...
                    if (this.email_missing_docs || this.email_to_crif || this.send_documents) {
    ...
    insertTemplate: function (template) {
        ...
        if (this.email_missing_docs) {
            ...
            $('.alert-btn-confirm').click();
        } else if (this.send_documents) {
            /**
            * CRED-760 : parse send document email template and update $current_user variable
            */
            var email_body = template.attributes.body_html;
            var current_user_name = app.user.get('full_name');
            /**
             * CRED-994 : Provider-Handling
             * Code breaking in case of empty email body
             */
            if (!_.isUndefined(email_body) || !_.isEmpty(email_body)) {
                email_template_body = email_body.replace('$current_user', current_user_name);
                self.model.set('html_body', email_template_body);
            }
            $('.alert-btn-confirm').click();
        } else if(self.assigned_user_id){
            var leadId = self.context.parent.get('model').get('id');
            ...
            */
            ...
            var url = App.api.buildURL("Emails/signatureParserApi/" + template.get("id") + "/" + self.assigned_user_id+'/'+self.language + "/" + leadId, null, null);
            App.api.call('read', url, null, {
                success: function (response) {
                    ...
                    /**
                     * CRED-896 : [Upgrade] Email subject not populating
                     */
                    if (subject) {
                        self.model.set('subject', subject);
                    } else {
                        self.model.set('subject', '');
                    }
                },
                error: function (error) {
