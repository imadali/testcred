    ...
    protected function formatResult(ServiceBase $api, array $args, SugarQuery $query, SugarBean $bean = null)
    {
        ...
        foreach ($data as $row) {
            ...
            if ($args['module'] == 'KBContents') {
                /**
                 * CRED-901 : [Upgrade]Change in Body text is not 
                 * being shown in Activity stream
                 */
                if ($row['activity_type'] == 'update' && isset($content->changes->kbdocument_body) && $content->changes->kbdocument_body->data_type == 'longtext') {
