...
class addRelatedNote {
     ...
     */
    public function addRelatedNoteToActivities($bean, $event, $arguments) {
        require_once 'modules/ModuleBuilder/views/view.modulefields.php';
        if ($arguments['related_module'] == 'Notes' && !empty($arguments['related_id']) && !empty($arguments['related_name']) && !empty($bean->leads_opportunities_1leads_ida) && $arguments['relationship'] == 'opportunity_notes') {
            $parent = BeanFactory::getBean("Leads", $bean->leads_opportunities_1leads_ida);
