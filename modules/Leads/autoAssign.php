...
class autoAssign
{
            ...
            if (isset($check[0]) && isset($check[4]) && $check[0] == "v10" && $check[4] == "leads") {
                ...
                if ($sugar_config['auto_assign']) {
                    if (empty($bean->dotb_correspondence_language_c)) {
                        ...
                        $id_user = $this->selectUser($bean->birthdate, $bean->dotb_correspondence_language_c, $bean->lead_type_id_c, $bean->first_name, $bean->last_name, $bean->input_process_type_id_c);
    ...
    /**
     * @param string $birthdate     birthdate of contact
     * @param string $language      Langue du contact
     * @param string $lead_type     Categorie de la CR (A, B ou NULL))
     * @param int    $first_name    first name of lead
     * @param int    $last_name     last name of lead
     * @param string $input_process Lead input process (CRED-991 : Lead-Assignment-Rule)
     *
     * @return string Id de l'utilisateur
     */
    private function selectUser($birthdate, $language, $lead_type, $first_name, $last_name, $input_process) 
    {
        ...
        /**
         * CRED-1032 : Handling Input-Process with Ref to Input-Channels (If day is 7 then 0 should be set for Sugar. If day is 8 then 1 as it means day was sunday and 1 was added so monday should be considered for auto assignment.)
         */
        if ($day == '7') {
            $day = '0';
        }
        ...
        $GLOBALS['log']->debug("Select contact on the basis of birthdate: $birthdate, language: $language, lead_type: $lead_type, first_name: $first_name, last_name: $last_name");
        if (empty($birthdate)) {
        ...
        if (!empty($contact_assigned_user_id)) {
            ...
            // Added check for input process CRED-991 : Lead-Assignment-Rule			
            if ($myUser->status == 'Active' && $myUser->dotb_is_active && strpos($myUser->input_process_type, '^' .$input_process . '^') !== false) {
        ...
        if (empty($selected_user_id)) {
            ...
            // Added check for input process CRED-991 : Lead-Assignment-Rule
            $query = "SELECT id as user_id FROM users WHERE status='Active' AND dotb_is_active = 1 AND dotb_working_days LIKE '%^" . $day . "^%' AND dotb_spoken_languages LIKE '%^" . $language . "^%' AND input_process_type LIKE '%^" . $input_process . "^%' AND deleted=0";
            $rs = $db->query($query);
            /*
             * If there is one assign to him, if there is none assign to services account.
             */
            if ($rs->num_rows == 1) {
        ...
        foreach ($last_option as $key => $value) {
            if (empty($selected_user_id)) {
                ...
                if ($value == 1) {
                    // Added check for input process CRED-991 : Lead-Assignment-Rule
                    $all_users_q = "SELECT id as user_id FROM users WHERE  status='Active' AND dotb_is_active = 1  AND dotb_working_days LIKE '%^" . $day . "^%' AND dotb_spoken_languages LIKE '%^" . $language . "^%' AND input_process_type LIKE '%^" . $input_process . "^%' AND deleted=0";
                } elseif ($value == 2) {
