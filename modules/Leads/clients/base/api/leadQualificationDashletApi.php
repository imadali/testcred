...
/**
 * Class for Lead Qualification Dashlet API
 */
class leadQualificationDashletApi extends SugarApi
{
    ...
    /**
     * Function for Calculation of Fields to be displayed in the dashlet
     * 
    ...
    public function getMissingFields(ServiceBase $api, array $args)
    {
        ...
        if (file_exists("custom/modules/Leads/clients/base/views/record/record.php")) {
            include_once 'custom/modules/Leads/clients/base/views/record/record.php';
        } else {
            include_once 'modules/Leads/clients/base/views/record/record.php';
        }
                ...
                foreach ($value['fields'] as $k => $v) {
                    ...
                    foreach ($requiredFields as $rk => $rv) {
                        ...
                        if (isset($v['name']) && ($v['name'] == $rv || $v == $rv)) {
                            ...
                            } elseif ($rv == "no_of_dependent_children_c") {
                                /**
                                 * CRED-935 : Upgrade - Color in Lead-Q-Dashlet for Kids
                                 */
                                if ($leadBean->no_of_dependent_children_c < 0 || $leadBean->no_of_dependent_children_c === ''
                                ) {
                                    $vname = $fieldDefs[$v['name']]['vname'];
                                    $emptyFields[] = $vname;
                                }
         ...
         */
        return array('panels' => $panelNames, 'deltavista_riskclass' => $leadBean->dv_risk_class,
            'deltavista_score' => $leadBean->deltavista_score_c);
    }
