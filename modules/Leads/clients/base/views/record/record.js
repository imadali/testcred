({
    ...
    initialize: function (options) {
        ...
        app.events.on('saveLead', _.bind(this.saveLead, this));
        ...
        ** CRED-887 : used _bind
        **/
        this.context.on('button:dotb_cembra_pdf_export_button:click', _.bind(this.initializePdfGeneration, this));
        this.context.on('script:lead:retrieved', _.bind(this.checkLaunchPDFGen, this));
        this.context.on('script:related_partner_contact:retrieved', _.bind(this.checkLaunchPDFGen, this));
        ...
        ** CRED-902
        */
        this.model.on('data:sync:complete', _.bind(this.makeReadOnly, this));
         ...
         *  -> New Button for Bank Now Car
         */
        this.context.on('button:generate_briefing_pdf_bank_now_car:click', this.generateBankNowCarBriefingPDF, this);
        /**
         * CRED-873 : CTI Basic: RT Action Items
         */
        this.model.on('data:sync:complete', this.changeFocus, this);
        app.events.on('refreshLeadStatus', _.bind(this.refreshLeadStatus, this));
    },
	...
	app.events.off('refreshLeadSubpanelOnLead');
        app.events.off('refreshLeadStatus');
        ...
        this.model.off('data:sync:complete');
        this.off('render');
            ...
            if (this.meta && this.meta.panels) {
                ...
                _.each(panels, function (panel) {
                    _.each(panel.fields, function (field, index) {
                        if (field.name !== 'credit_request_substatus_id_c' ||
                                (field.name === 'credit_request_substatus_id_c' && this.model.get('credit_request_substatus_id_c') != "")) {
                            field.readonly = true;
                        }
    ...
    /**
    * CRED-816 : Save lead button clicked when lead status is 11 and user has selected application in task drawer
    */
    saveLead: function () {
    ...
    generatePdf: function (latest_related_address) {
        ...
        var model = {
            ...
            /* Removed as this field is to remain empty CRED-800 
        ...
        if (contactData.dotb_is_home_owner_c == 'yes') {
            model.contact_dotb_housing_costs = '';
        }

        if (contactData.dotb_employment_type_id_c == 'fixed_term' || contactData.dotb_employment_type_id_c == 'temporary_contract' || contactData.dotb_employment_type_id_c == 'self_employed' || contactData.dotb_employment_type_id_c == 'disabled_gets_pension' || contactData.dotb_employment_type_id_c == 'retirement') {
            model.contact_dotb_has_thirteenth_salary = '';
        }

        if (contactData.dotb_employment_type_id_c == 'disabled_gets_pension' || contactData.dotb_employment_type_id_c == 'retirement') {
            model.truncated_contact_dotb_employed_since = '';
        }

        if (contactData.dotb_has_second_job_c != 'yes') {
            model.contact_dotb_second_job_has_13th = '';
            model.contact_dotb_second_job_gross_income = '';
        }
        // End CRED-948 : Empty fields for PDF
        ...
        /** 
        * updated condition as per CRED-800
        */
        model.calculate_contact_housing_situation_parents = (contactData.dotb_housing_situation_id_c == 'by_parents' && (contactData.dotb_is_home_owner_c == 'yes' || contactData.dotb_is_home_owner_c == 'no'));

        /** 
        * updated condition as per CRED-800
        */
        model.calculate_contact_housing_situation_mortgage = '';
        if((contactData.dotb_housing_situation_id_c == 'married_couple' && contactData.dotb_is_home_owner_c == 'yes') || (contactData.dotb_housing_situation_id_c == 'flat_share' && contactData.dotb_is_home_owner_c == 'yes' && contactData.dotb_is_rent_split_c == 'no') || (contactData.dotb_housing_situation_id_c == 'alone' && contactData.dotb_is_home_owner_c == 'yes') || ((contactData.dotb_housing_situation_id_c == 'single_parent' && contactData.dotb_is_home_owner_c == 'yes')))
            model.calculate_contact_housing_situation_mortgage = '1';

        /** 
        * updated condition as per CRED-800 and CRED-879
        */
        model.calculate_contact_housing_situation_shared = '';
        if((contactData.dotb_housing_situation_id_c == 'flat_share' && contactData.dotb_is_home_owner_c == 'no' && contactData.dotb_is_rent_split_c == 'no') || (contactData.dotb_housing_situation_id_c == 'married_couple' && contactData.dotb_is_home_owner_c == 'no') || (contactData.dotb_housing_situation_id_c == 'single_parent' && contactData.dotb_is_home_owner_c == 'no') || (contactData.dotb_housing_situation_id_c == 'alone' && contactData.dotb_is_home_owner_c == 'no'))
        ...
        */
		...
		if(contactData.dotb_housing_situation_id_c == 'flat_share' && contactData.dotb_is_home_owner_c == 'no' && contactData.dotb_is_rent_split_c == 'yes')
		    model.calculate_contact_housing_situation_shared_part = '1';
...
//        }, this));
    ...
    rciPortal: function () {
        ...
        app.api.call('create', app.api.buildURL('Leads/rciPortal/' + lead_id), {lead_id: lead_id, module: 'Leads'}, {
            success: function (rciForm) {
                app.alert.dismissAll();
                if (rciForm.success == 'ok') {
                    $(".rci_portal_form").remove();
                    $(rciForm.details).appendTo('body').submit();
                } else if (rciForm.success == 'notok') {
                    app.alert.show("rci-form-error", {
                        level: 'error',
                        messages: rciForm.details,
                        autoClose: false
                    });
                }
            },
     ...
     */
    generateBankNowCarBriefingPDF: function () {
        app.alert.show("app-process", {
            level: 'process',
            messages: 'Loading',
            autoClose: true
        ...
        app.api.call(
                'create',
                app.api.buildURL('Leads/generateBriefingPdf/bank_now_car'),
                {
                    lead_id: this.model.id
                },
        {
    ...
    /**
     * CRED-873 : CTI Basic: RT Action Items
     * CRED-964 : The screen not moving to Activities subpanel after Workflow execution
     */
    changeFocus: function () {
        var parentElementId = $('[data-panelname="LBL_RECORDVIEW_PANEL8"]').parent().attr("id");
        if (!_.isUndefined(parentElementId) && this.model.get('credit_request_status_id_c') !== '11_closed') {
            var parent = parentElementId.split("view")[0];
            $('.tab.' + parent + ' a').trigger('click');
            $('.main-pane').scrollTop(540);
            this.model.off('data:sync:complete', this.changeFocus, this);
        }
    },
