...
({
    ...
    handleActivityPreview: function (model) {
        /* if(model.get('parent_type') == 'Emails') {
         emailModel = App.data.createBean('Emails', {
         id: model.id
         });
         emailModel.fetch({
         success: function() {
         var url = App.api.buildURL('CEmail/GetEmail');
         App.api.call('create', url, {
         id: model.id,
         }, {
         success: _.bind(function(response) {
         for (var prop in response) {
         emailModel.set(prop,response[prop]);
         }
         App.drawer.open({
         view: 'emaildetail',
         context: {
         ...
         app.api.call(
         'read',
         app.api.buildURL(model.get('parent_type'),'read',{id:model.get('parent_id')}),
         null,
         {
