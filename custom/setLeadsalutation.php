...
$sql = "SELECT id, dotb_correspondence_language_c,dotb_gender_id_c,salutation_text_c FROM leads JOIN leads_cstm ON id=id_c where deleted=0";
$result = $GLOBALS["db"]->query($sql);
echo "Seting Salutation for Leads";
$count=1;
$queries='';
while ($lead = $GLOBALS["db"]->fetchByAssoc($result)) {
$corresponding_language = $lead['dotb_correspondence_language_c'];
if (!empty($corresponding_language)) {
    $id = $lead['id'];
    $gender = $lead['dotb_gender_id_c'];
    if(empty($gender)){
    ...
    $sql = "UPDATE leads SET salutation_text_c = '$salutation_text_c' where id='$id';";
