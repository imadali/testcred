...
({
    ...
    bindKeyDown: function(callback) {
        ...
        if ($el) {
            $el.on('keydown.record', {
                field: this
            }, callback);
            var plugin = $el.data('select2');
            if (plugin) {
                if (plugin.focusser) {
                    plugin.focusser.on('keydown.record', {
                        field: this
                    }, callback);
                }
                plugin.search.on('keydown.record', {
                    field: this
                }, callback);
            }
        }
    ...
    _checkForDefaultValue: function(currentValue, optionsKeys) {
        ...
        if (!this.def.isMultiSelect && !_.isEmpty(this.items) && !(this.model.has(this.name) && optionsKeys.indexOf(currentValue) > -1) && app.acl.hasAccessToModel('write', this.model, this.name) && (action == 'edit' || action == 'create')) {
    ...
    getSelect2Options: function(optionsKeys) {
        ...
        if (this.def.dropdown_width) {
            select2Options.dropdownCss = {
                width: this.def.dropdown_width
            };
        }
    ...
    _filterOptions: function(options) {
        var currentValue, syncedVal, newOptions = {},
    ...
    _setupKeysOrder: function() {
        var keys, orderedKeys, filteredOrderedKeys, toString = function(k) {
            return k.toString();
        };
        if (!this._keysOrder) {
            this._keysOrder = {};
