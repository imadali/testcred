...
({
    ...
    _render: function() {
        ...
        if (this.tplName === 'edit') {
			...
			_.each(this.value, function(email) {
				...
				//emailsHtml += this._buildEmailFieldHtml(email);
				//console.log("in render function: "+emailsHtml);
    ...
    _addNewAddressToModel: function(email) {
        ...
        if (_.isUndefined(dupeAddress)) {
			...
			//	console.log("from model: "+ existingAddresses);
			...
			//	console.log("here "+this.name+" "+existingAddresses);
			this.model.set(this.name, existingAddresses);

			//	console.log("after setting.from model: "+ this.model.get(this.name) ? app.utils.deepCopy(this.model.get(this.name)) : []);
    ...
    _updateExistingAddressInModel: function(index, newEmail, oldAddress) {
		...
		//var re_oldAddress = new RegExp(oldAddress, 'g');
		...
		for(var i=0 ; i < addressArray.length ; i++)
		{
			...
			// if(addressArray[i] === oldAddress)
			// {
				// addressArray[i] = newEmail;
			// }
		...
		//var newAddress = existingAddresses.replace(oldAddress, newEmail);
		//console.log("setting new address: "+existingAddresses);
