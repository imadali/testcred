...
/**
* CRED-937
* (https://developer.sugarcrm.com/2017/07/26/triggering-advanced-workflow-processes-in-php-customizations/)
*/
...
function getCreditCheckResponse()
{
    ...
    $query = "SELECT leads.id, leads_cstm.deltavista_request_id_c as request_id
                  FROM leads LEFT JOIN leads_cstm ON leads.id=leads_cstm.id_c
                    ...
                    AND leads_cstm.has_deltavista_response_c = 'no' ORDER BY date_modified ASC";
    ...
    while ($row = $db->fetchByAssoc($rs)) {
        ...
        if ($result_data['Status'] == 'Response') {
            ...
            if (!empty($leadBean->id)) {
                ...
                /**
                 * CRED-937 : Workaround so that Process Author is executed on all records saved concurrently (https://developer.sugarcrm.com/2017/07/26/triggering-advanced-workflow-processes-in-php-customizations/). Moved registry clear before bean save
                 */
                ...
                /**
                * CRED-767 RiskClass saved in leads
                */
                if (is_array($debt_collection)) {
                    if (isset($debt_collection[0]['RiskClass']))
                    $leadBean->dv_risk_class = $debt_collection[0]['RiskClass'];
                }
