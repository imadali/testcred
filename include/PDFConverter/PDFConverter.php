...
class PDFConverter {
    ...
    function resizeImage($sourceImage, $targetImage, $maxWidth, $maxHeight, $quality = 80) {
        if(!empty($sourceImage)) {
            ...
            if (isset($size['0']) && isset($size['1']) && $size['0'] > 1024 && $size['1'] > 1024 && ($size['mime'] == 'image/jpeg' || $size['mime'] == 'image/png')) {
                /**
                ** Added check that only jpeg and png images are resized
                **/
    ...
    public function archievedPDFMerged($pdfFiles, $upload_path) {
        ...
        try {
            ...
            foreach ($pdfFiles as $key => $pdfFiles) {
                ...
                $inDocument->setCacheReferencedObjects(true);
                /** Check that the document loaded is security enabled or not.
                ** If security enabled throw exception
                **/
                if ($inDocument->hasSecHandler()) {
                    throw new SetaPDF_Core_SecHandler_Exception(
                        'Security handler is attached to this document',
