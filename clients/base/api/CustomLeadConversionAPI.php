...
class CustomLeadConversionAPI extends SugarApi
{
    ...
    public function createContract($api, $args)
    {
        ...
        if ($approvedCount == 1 && $approvedAppObj != null) {
             ...
             * Sending values from FLEX fields if provider is FLEX
             */
            if ($approvedAppObj->provider_id_c == 'bank_now_flex') {
                $arr['interest_rate'] = $approvedAppObj->approved_interest_rate_flex;
                $arr['credit_amount'] = $approvedAppObj->approved_credit_amount_flex;
                $arr['credit_duration'] = $approvedAppObj->approved_credit_duration_flex;
                
                /**
                 * To seperate Flex values from normal values
                 */
                $arr['customer_credit_amount_flex'] = $approvedAppObj->contract_credit_amount_flex;
                $arr['customer_credit_duration_flex'] = $approvedAppObj->contract_credit_duration_flex;
                $arr['customer_interest_rate_flex'] = $approvedAppObj->contract_interest_rate_flex;
                $arr['customer_ppi_c'] = $approvedAppObj->contract_ppi_flex;
                $arr['customer_contract_first_payment_flex'] = $approvedAppObj->contract_first_payment_flex;
                $arr['soko'] = $approvedAppObj->soko_flex;
                if ($approvedAppObj->approved_ppi_flex) {
                    $arr['ppi'] = 'PPI';
                } else {
                    $arr['ppi'] = 'NOPPI';
                }
    ...
    public function createContact($api, $args)
    {
        ...
        if ($contactObj) {
            ...
            include_once 'custom/modules/Leads/syncLead.php';
    ...
    public function createLead($api, $args)
    {
        ...
        /**
         * CRED-971 : Show Evalanche Campaigns instead of Sugar campaigns
         */
        if (!empty($args['lead_campaign'])) {
            $leadObj->load_relationship('kevacampaigns_leads_link');
            $leadObj->kevacampaigns_leads_link->add($args['lead_campaign']);
        }
