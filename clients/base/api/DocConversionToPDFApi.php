...
class DocConversionToPDFApi extends SugarApi {
    ...
    public function mergeSelectedPagesIntoPDF(ServiceBase $api, $args) {
        ...
        $response = $pdfObj->mergeSelectedPagesIntoPDF($args);

        if (isset($response['level']) && $response['level'] == 'success') {
            global $current_user, $timedate;
            $pages = array();
            $result = '';
            $audit_insert = '';
            foreach ($args['pdf_info'] As $key => $value) {
                if (sizeof($args['pdf_info'] > 1)) {
                    $pages[] = $value['page_number'];
                } else {
                    $pages = $value['page_number'];
                }
            }
            ...
            $pages = implode(', ', $pages);
            $result = $GLOBALS['db']->query("SELECT document_name FROM documents WHERE id = '" . $args['pdf_info'][0]['document_id'] . "'");
            $row = $GLOBALS['db']->fetchByAssoc($result);

            $currenrDateTime = $timedate->getInstance()->nowDb();
            $date = date('Y-m-d H:i:s', strtotime($currenrDateTime));

            $audit_insert = 'INSERT INTO leads_audit' . '
                    (id, parent_id, date_created, data_type, created_by, field_name, before_value_string, after_value_string) 
                    VALUES ("' . create_guid() . '", "' . $args['record_id'] . '","' . $date . '","doc_merge", "' . $current_user->id . '", "doc_merge","' . $row['document_name'] . '","' . $pages . '")';
